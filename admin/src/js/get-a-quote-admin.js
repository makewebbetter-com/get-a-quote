...
const label =
  '<label for="{{fields-id-here}}" id="{{field-id-lname}}" data-id="{{field-lname}}" class="form-labels">{{fields-label-here}}</label>';

const icons =
  '<a class="mwb_gaq__icon--del"><i id="{{fields-scope}}" class="fas fa-trash-alt icon_del"></i></a><a class="mwb_gaq__icon--edit"><i data-id="{{scope}}" class="far fa-edit icon_edit"></i></a>';

const wrapper =
  '<div id="form-group-{{fields-scope-here}}" class="mwb_gaq__form--group"></div>';
...
function appendNewlement(attrs = []) {
  ...
  if (jQuery(newElement)[0].id == "fcountry") {
    jQuery.ajax({
      type: "POST",
    ...
    $(jQuery("#append-form").children()).each(function(index) {
      $($(this).children()).each(function(inde) {
        if ($(this)[0].localName != "svg") {
          ...
          if (
            ($(this)[0].placeholder != undefined && $(this)[0].id != "") ||
            $(this)[0].localName == "select"
          ) {
...
jQuery(document).ready(function($) {
  var data = taxonomy_values.converted;
  ...
  var dataforpreview = taxonomy_values.converted;
  ...
  if (dataforpreview != null && dataforpreview != "") {
    if ($(".mwb_display_form").length > 0) {
      ...
      jQuery(".mwb_display_form ").append(
        '<a href="#" class="mwb-btn btn-info-mwb">Submit</a>'
      );
    }
  }
});
...
jQuery(document).ready(function($) {
  $("#copytoclipTxt").on("click", function() {
    ...
    $(".mwb-gaq__alert").css("visibility", "visible");
    setTimeout(function() {
      $(".mwb-gaq__alert").css("visibility", "hidden");
    }, 1000);
  });
});

jQuery(document).ready(function($) {
  var ty = $("#mwb_gaq_taxonomies_common_settings_save");

  if (ty[0] != undefined) {
    $(ty[0].parentElement).addClass("taxonomy_button");
  }
  ...
  if (status != "") {
    for (i = 0; i < status.length; i++) {
      if (status[i].name == "Pending") {
        $(".mwb_gaq_status_terms ").append(
          "<td class='mwb_active_terms_status' id=" +
            status[i].term_id +
            ">" +
            status[i].name +
            "</td>"
    ...
    for (i = 0; i < service.length; i++) {
      if (service[i].name == "Quotation") {
        $(".service_terms ").append(
          "<td class='mwb_active_terms_service' id=" +
            service[i].term_id +
            ">" +
            service[i].name +
            "</td>"
  ...
  var redirect_value = taxonomy_values.redirect;
  if (redirect_value == "No") {
    $(".redirection").hide();
  }

  $("#select_for_redirection").on("change", function() {
    if ($("#select_for_redirection :selected").val() == "Yes") {
      $(".redirection").show();
    } else {
      $(".redirection").hide();
    }
